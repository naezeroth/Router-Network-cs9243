...
extendedNetworkGraph2() ->
  [{red  , [{white, [grey, white, green, black]},
      {blue , [blue]}]},
   {white, [{red, [blue]},
      {blue, [green, red]},
      {black, [black, grey]}]},
   {blue , [{green, [white, green, red, black, grey]}]},
   {green, [{red, [red, blue, white, black, grey]}]},
   {black, [{red, [red, green]}, {green, [white, blue]}, {grey, [grey]}]},
   {grey, [{black, [red, white, green, blue, black]}]}
  ].
...
runTest () ->
  ...
  {BlackPid , _} = networkTest:probeNetwork (WhitePid, black),
  io:format ("*** Extending network from black...~n"),
  case control:extendNetwork (WhitePid, 2, black, 
               {grey, [{BlackPid, [red, white, green, blue, black]}
                  ]})
    of
    true  -> io:format ("*** ...done.~n");
    false -> io:format ("*** ERROR: Extension failed!~n")
  end,
  networkTest:verifyNetwork (RedPid, extendedNetworkGraph2()).
